...
1. ユーザーがセッション画面の設定セクションでBPMスライダーを操作したとき、システムは設定値を40〜120 BPMの範囲・1刻みで保存する。
...
4. セッションを新規開始するとき、システムは直近保存したBPM・時間設定を自動適用する。
5. ユーザーが設定時間を60〜300秒の範囲で変更する、または「無制限」を選択したとき、システムはその値（無制限の場合は上限なし）を保存し、いつでも手動停止を受け付ける。
6. ユーザーが「デフォルトに戻す」を選択したとき、システムは心拍設定（BPM=60、時間=180）を即時反映し保存する。
...
3. 設定時間を経過するかユーザーが手動停止したとき、システムはガイドを停止する。
4. ユーザーが呼吸ガイドを開始したとき、システムは吸う/止める/吐く各フェーズ開始タイミングで振動を出力し、視覚ガイドをフェーズに同期して表示する（BPM設定の影響を受けない）。
...
**Objective:** ユーザーとして、セッション前後の状態を記録し、後から効果を振り返り、必要に応じて履歴を整理したい。

#### Acceptance Criteria (EARS日本語表現)
1. ユーザーがセッション開始前に心拍数を入力したとき、システムは値を記録し、未入力でも開始できるようにする。
2. セッション実行前/中/停止後のいずれでも、ユーザーが記録を選択して心拍数（開始/終了）を入力したとき、システムは各入力値を保存する。
3. 記録時、システムは使用ガイド種別（振動/呼吸）をユーザー選択で保存する。
4. 記録時、システムは改善度を5段階で入力・保存できるようにする。
5. ユーザーが履歴一覧を閲覧するとき、システムは各セッションの開始/終了心拍・ガイド種別・改善度を表示し、記録時刻の最新順で並べる。
6. ユーザーが履歴一覧の末尾までスクロールしたとき、システムは次の履歴を追加で読み込み、既存の一覧に追記する。
7. ユーザーが履歴タブを初めて表示したとき（初回フォーカス時）、システムは最新順の履歴を取得して表示する。
8. ユーザーが履歴タブを再表示したとき、システムは自動で再取得せず、直前の表示内容を維持する。
9. ユーザーが履歴一覧で下に引っ張って更新を実行したとき、システムは最新順の履歴を再取得し、既存一覧に重複なく追加して更新する（空の履歴表示時も更新操作を実行できる）。
10. ユーザーが履歴タブで「選択」を開始したとき、システムは複数選択用のUI（チェックボックス等）を表示する。
11. ユーザーが複数の履歴を選択したとき、システムは選択件数を表示し、削除操作を有効化する。
12. ユーザーが削除を実行したとき、システムは確認ダイアログで件数を提示し、確認後に選択した履歴を一括削除する。
13. 削除完了後、システムは履歴一覧から削除対象を即時に除外する。
14. ユーザーが選択を解除またはキャンセルしたとき、システムは選択モードを解除し、通常の一覧表示に戻す。
