...
- [ ] 2.3 モード選択後に即時開始/停止でガイドを確認できるようにする（プレビュー廃止）
  - 振動モード: 1拍1振動を現在のBPM/強度で再生（Webは無振動でエラーなし）、円アニメでパルスを表示
...
- [ ] 8. ナビゲーション（タブ）再構成
- [ ] 8.1 タブを「セッション」「履歴」の2つに限定し不要タブを削除する
  - app/(tabs)/_layout.tsx を更新し、Home/Exploreを廃止して Session/Logs を定義する
  - _Requirements: 5.1_
- [ ] 8.2 デフォルトタブを「セッション」に設定し1タップで開始操作へ導く
  - initialRouteName を session に設定し、戻る動線でもセッションへ戻ることを確認する
  - _Requirements: 5.2_
- [ ] 8.3 履歴タブから最新順一覧へ1タップで遷移できるようにする
  - tabs→logs で一覧が即表示されるようルーティングを修正し、表示データを最新順で取得する
  - _Requirements: 5.3_
